...
////////////////////////////////////////////////////////////////////
int WavInFile::readbyte(static unsigned char *B, int num) {
	int numBytes;
	uint afterDataRead;
	numBytes = num;
	afterDataRead = dataRead + numBytes;
	...
	assert(B);
	numBytes = (int)fread(B, 1, numBytes, fptr);
	dataRead += numBytes;
	return numBytes;
}
void WavOutFile::writebyte(static unsigned char *B, int num) {
	int res;
	res = (int)fwrite(B, 1, num, fptr);
	if (res != num)
	{
		ST_THROW_RT_ERROR("Error while writing to a wav file.///custom");
	}
