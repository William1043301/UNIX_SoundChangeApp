...
void reverseB(char *in) {
	WavInFile *inF;
	inF = new WavInFile(in);
	WavOutFile *out;
	out = new WavOutFile("reverse.wav", (int)inF->getSampleRate(), (int)inF->getNumBits(), (int)inF->getNumChannels());
	static unsigned char* BUFF;
	vector<int>rea;///byte read [red]
	vector<unsigned char*>b;
	int numread = 0;
	while (inF->eof() == 0) {
		BUFF = new unsigned char[BUFF_SIZE];
		numread=inF->readbyte(BUFF, BUFF_SIZE);
		rev(BUFF, numread,((inF->getNumBits())/8));
		//out->writebyte(BUFF, numread);
		rea.push_back(numread);
		b.push_back(BUFF);
	}
	for (int i = b.size() - 1; i > -1; i--/*int i=0;i!=b.size();i++*/)
	{
		out->writebyte(b[i], rea[i]);
	}
	delete out;
	delete inF;
}
